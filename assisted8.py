...
# Function to listen for the next session command
def listen_for_next_session():
    recognizer = sr.Recognizer()
    with sr.Microphone() as source:
        st.info("Say 'new session' to start again or 'ask question' to ask another question about the same image...")
        speak_text("Say 'new session' to start again or 'ask question' to ask another question about the same image.")
        recognizer.adjust_for_ambient_noise(source, duration=2)
        try:
            st.info("Listening for command... Speak now!")
            audio = recognizer.listen(source, timeout=10, phrase_time_limit=3)
            command = recognizer.recognize_google(audio).lower()
            st.write(f"Detected Command: {command}")  # Debugging: Show detected speech
            
            if "new" in command or "session" in command or "start" in command or "again" in command:
                return "new_session"
            elif "ask" in command or "question" in command or "another" in command:
                return "ask_question"
            else:
                speak_text("Command not recognized. Please say 'new session' or 'ask question'.")
                return listen_for_next_session()  # Recursive call to try again
        except sr.WaitTimeoutError:
            st.error("Timeout: No speech detected.")
        except sr.UnknownValueError:
            st.error("Could not understand audio.")
            ...
            st.error("Speech recognition service error.")
    return None  # Return None if no valid command is detected
...
if st.session_state.app_stage == 'start':
    ...
    command = listen_for_next_session()
